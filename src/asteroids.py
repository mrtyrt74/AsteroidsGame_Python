...
class Asteroids:
    ...
    def __init__(self):
        ...
        self.numRocks = None
        self.fps = None
        self.explodingCount = None
        self.nextLife = None
        self.startLives = None
    ...
    def createRocks(self, numRocks):
        for _ in range(0, numRocks):
            ...
            newRock = badies.Rock(self.stage, position, badies.Rock.largeRockType)
    ...
    def doSaucerLogic(self):
        ...
        if self.secondsCount % 2000 == 0 and self.saucer is None:
            ...
            if randVal <= 3:
                self.saucer = badies.Saucer(
                    self.stage, badies.Saucer.smallSaucerType, self.ship)
            else:
                self.saucer = badies.Saucer(
                    self.stage, badies.Saucer.largeSaucerType, self.ship)
    ...
    def checkCollisions(self):
        ...
        for rock in self.rockList:
            ...
            if rockHit:
                ...
                if rock.rockType == badies.Rock.largeRockType:
                    playSound("explode1")
                    newRockType = badies.Rock.mediumRockType
                    self.score += 50
                elif rock.rockType == badies.Rock.mediumRockType:
                    playSound("explode2")
                    newRockType = badies.Rock.smallRockType
                    self.score += 100
                else:
                    playSound("explode3")
                    self.score += 200

                if rock.rockType != badies.Rock.smallRockType:
                    # new rocks
                    for _ in range(0, 2):
                        position = Vector2d(rock.position.x, rock.position.y)
                        newRock = badies.Rock(self.stage, position, newRockType)
                        self.stage.addSprite(newRock)
                        self.rockList.append(newRock)
    ...
    def createDebris(self, sprite):
        for _ in range(0, 25):
            ...
            debris = badies.Debris(position, self.stage)
